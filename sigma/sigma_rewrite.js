...
------------------------------------------
@Date: 2026.08.22
@Description: Sigma 卡路里 — 重写拦截存凭证
------------------------------------------
new Env("Sigma卡路里-存凭证");
...
async function getCookie() {
  try {
    ...
    // 排除redeem/list查询接口
    if (url.indexOf("/redeem/list") !== -1) {
      $.info("redeem/list查询接口，放行");
      return;
    }
    ...
    $.msg($.name, `🎉凭证更新成功!`, `URL: ${url.substring(url.lastIndexOf("/"))}\nbody: ${reqBody.substring(0, 100)}\nauth: ${auth}...`);
    $.info(`凭证已存储到 ${ckName}`);
  } catch (e) {
